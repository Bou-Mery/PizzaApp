...
import ma.ensa.tp2.adapter.PizzaAdapter;
import ma.ensa.tp2.classes.Produit;
import ma.ensa.tp2.service.ProduitService;
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        ps.create(new Produit(
                ...
                "So fast and so easy with refrigerated pizza crust, these saucy, smoky pizzas make quick fans with their hot-off-the-grill, rustic flavor. "
                        + "They're perfect for spur-of-the-moment cookouts and summer dinners on the patio. —Alicia Trevithick, Temecula, California",
                "STEP 1:\n\n"
                        + "Sprinkle chicken with pepper; place on an oiled grill rack over medium heat. Grill, covered, until a thermometer reads 165°, 5-7 minutes per side, "
                        + "basting frequently with 1/2 cup barbecue sauce during the last 4 minutes. Cool slightly. Cut into cubes.\n\n"
                        + "STEP 2:\n\n"
                        + "Divide dough in half. On a well-greased large sheet of heavy-duty foil, press each portion of dough into a 10x8-in. rectangle; brush lightly with oil. "
                        + "Invert dough onto grill rack; peel off foil. Grill, covered, over medium heat until bottom is lightly browned, 1-2 minutes.\n\n"
                        + "STEP 3:\n\n"
                        + "Remove from grill. Spread grilled sides with remaining barbecue sauce. Top with cheese, chicken and onion. "
                        + "Grill, covered, until bottom is lightly browned and cheese is melted, 2-3 minutes. Sprinkle with cilantro. Yield: 2 pizzas (4 pieces each)."
        ));
        ...
        list.setOnItemClickListener(new AdapterView.OnItemClickListener() {
            ...
            public void onItemClick(AdapterView<?> adapterView, View view, int i, long l) {
                Produit selectedProduit = ps.findById(i);

                Intent intent = new Intent(Pizza_list.this , PizzaInformation.class);
                intent.putExtra("nom" , selectedProduit.getNom());
                intent.putExtra("foto" , selectedProduit.getPhoto()+"");
                intent.putExtra("descreption" , selectedProduit.getDescreption());
                intent.putExtra("ingrediant" , selectedProduit.getDetaisIngred());
                intent.putExtra("preparation" , selectedProduit.getPreparation());


               // Toast.makeText(Pizza_list.this , selectedProduit.toString()+"" ,Toast.LENGTH_LONG).show();
